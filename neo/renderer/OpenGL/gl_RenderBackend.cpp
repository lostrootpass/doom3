...
#include "gl_RenderBackend.h"
#include "../RenderParms.h"

void idRenderBackendGL::BindVariableStageImage(const textureStage_t *texture, const float *shaderRegisters)
{
...
void idRenderBackendGL::CopyRender(const void* data)
{
...
void idRenderBackendGL::DrawElementsWithCounters(const drawSurf_t* surf)
{
	// get vertex buffer
...
void idRenderBackendGL::DrawInteractions()
{
...
int idRenderBackendGL::DrawShaderPasses(const drawSurf_t * const * const drawSurfs, const int numDrawSurfs, const float guiStereoScreenOffset, const int stereoEye)
{
...
void idRenderBackendGL::DrawView(const void* data, const int stereoEye)
{
...
void idRenderBackendGL::DrawViewInternal(const viewDef_t* viewDef, const int stereoEye)
{
...
void idRenderBackendGL::FinishStageTexturing(const shaderStage_t *pStage, const drawSurf_t *surf)
{
...
void idRenderBackendGL::FillDepthBufferFast(drawSurf_t **drawSurfs, int numDrawSurfs)
{
	if ( numDrawSurfs == 0 ) {
		return;
	}
...
void idRenderBackendGL::FogAllLights()
{
...
void idRenderBackendGL::PostProcess(const void* data)
{
...
void idRenderBackendGL::PrepareStageTexturing(const shaderStage_t * pStage, const drawSurf_t * surf)
{
...
void idRenderBackendGL::SetMVP(const idRenderMatrix& mvp)
{
	SetVertexParms( RENDERPARM_MVPMATRIX_X, mvp[0], 4 );
}

void idRenderBackendGL::BakeTextureMatrixIntoTexgen(idPlane lightProject[3], const float *textureMatrix)
{
	float genMatrix[16];
	float final[16];
...
void idRenderBackendGL::BasicFog(const drawSurf_t *drawSurfs, const idPlane fogPlanes[4], const idRenderMatrix * inverseBaseLightProject)
{
	backEnd.currentSpace = NULL;
...
void idRenderBackendGL::BlendLight(const drawSurf_t *drawSurfs, const viewLight_t * vLight)
{
	backEnd.currentSpace = NULL;
...
void idRenderBackendGL::BlendLight(const drawSurf_t *drawSurfs, const drawSurf_t *drawSurfs2, const viewLight_t * vLight)
{
	if ( drawSurfs == NULL ) {
		return;
	}
...
void idRenderBackendGL::DrawSingleInteraction(drawInteraction_t * din)
{
...
void idRenderBackendGL::FillDepthBufferGeneric(const drawSurf_t * const * drawSurfs, int numDrawSurfs)
{
	for ( int i = 0; i < numDrawSurfs; i++ ) {
...
void idRenderBackendGL::FogPass(const drawSurf_t * drawSurfs, const drawSurf_t * drawSurfs2, const viewLight_t * vLight)
{
	renderLog.OpenBlock( vLight->lightShader->GetName() );
...
void idRenderBackendGL::GetShaderTextureMatrix(const float *shaderRegisters, const textureStage_t *texture, float matrix[16])
{
...
void idRenderBackendGL::LoadShaderTextureMatrix(const float *shaderRegisters, const textureStage_t *texture)
{	
...
void idRenderBackendGL::MotionBlur()
{
...
void idRenderBackendGL::RenderInteractions(const drawSurf_t *surfList, const viewLight_t * vLight, int depthFunc, bool performStencilTest, bool useLightDepthBounds)
{
	if ( surfList == NULL ) {
		return;
	}
...
void idRenderBackendGL::SetupForFastPathInteractions(const idVec4 & diffuseColor, const idVec4 & specularColor)
{
...
void idRenderBackendGL::SetupInteractionStage(const shaderStage_t *surfaceStage, const float *surfaceRegs, const float lightColor[4], idVec4 matrix[2], float color[4])
{
...
void idRenderBackendGL::StencilSelectLight(const viewLight_t * vLight)
{
...
void idRenderBackendGL::StencilShadowPass(const drawSurf_t *drawSurfs, const viewLight_t * vLight)
{
